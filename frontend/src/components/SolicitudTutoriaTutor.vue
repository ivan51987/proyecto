                    ...
                    <div class="flex items-start justify-between">
                      <div class="flex-1">
                        <div class="flex items-center">
                          <img
                            :src="solicitud.estudianteImagen || '/placeholder-avatar.png'"
                            alt="Estudiante"
                          ...
                          <div class="ml-4">
                            ...
                            <p class="text-sm text-gray-500">{{ solicitud.carrera }}</p>
                          </div>
                        </div>
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <p class="mt-1">{{ solicitud.areaInvestigacion }}</p>
                          </div>
                          <div>
                            <p class="text-sm font-medium text-gray-500">Fecha de Solicitud</p>
                            <p class="mt-1">{{ formatearFecha(solicitud.fechaSolicitud) }}</p>
                          </div>
                        </div>
  
                        <div class="mt-4">
                          <p class="text-sm font-medium text-gray-500">Motivo de la Solicitud</p>
                          <p class="mt-1 text-gray-700">{{ solicitud.motivo }}</p>
                        </div>
                      </div>
        ...
        <div class="bg-white rounded-lg p-6 max-w-lg w-full">
          ...
          <div>
            ...
            <textarea
              ...
              rows="4"
              class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
              :placeholder="modalTipo === 'aceptar' ? 'Escribe un mensaje de bienvenida...' : 'Explica el motivo del rechazo...'"
            ></textarea>
          </div>
  ...
  const solicitudes = ref([
    {
      ...
      estudianteImagen: null,
      areaInvestigacion: 'Desarrollo Web',
      fechaSolicitud: new Date('2024-01-15'),
      motivo: 'Me interesa desarrollar un proyecto de aplicación web usando tecnologías modernas.',
      estado: 'pendiente',
      respuesta: null
    },
    {
      ...
      motivo: 'Busco desarrollar un sistema de reconocimiento de patrones.',
      estado: 'aceptada',
      respuesta: 'Me parece un proyecto interesante. Podemos comenzar la próxima semana.'
    }
  ]);
  
  const solicitudesFiltradas = computed(() => {
    return solicitudes.value.filter(s => {
      const cumpleBusqueda = s.nombreEstudiante.toLowerCase().includes(busqueda.value.toLowerCase());
